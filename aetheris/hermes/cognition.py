...
INTENT_PROTOTYPES: dict[str, tuple[str, ...]] = {
    ...
    "video": ("generate video", "create video", "make a video", "animate", "animation", "text to video", "video clip"),
...
_CUE_PATTERNS: tuple[tuple[str, re.Pattern[str], float], ...] = (
    ...
    ("video", re.compile(r"(?:generate|create|make|render|animate).{0,60}\b(?:video|animation|clip|movie)\b|text[\s-]+to[\s-]+video", re.I), 0.90),
