...
function buildIndex(
  ...
  const routes: { path: string; label: string; blurb: string; tag?: string[] }[] = [
    ...
    { path: "/cores-health", label: "Core Health", blurb: "Live per-core health view: Section-3 build-call, capabilities owned, capabilities denied under your grants, modules rollup", tag: ["cores", "health", "section3"] },
    { path: "/accuracy", label: "Engine Accuracy", blurb: "NIRIKSHAN evaluation harness: per-class precision/recall/F1, confusion matrix, calibration, latency. With no real benchmark wired in, the page is honest: it does not invent accuracy", tag: ["accuracy", "evaluation", "nirikshan", "section2"] },
