...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const secret = new URL(req.url).searchParams.get("secret") ?? req.headers.get("x-aetheris-secret"); if (secret !== a.trigger.secret) return NextResponse.json({ error: "bad secret" }, { status: 401 });
