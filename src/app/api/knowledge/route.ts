...
import { addFact, fabricStatus, listFacts, queryFacts, type SourceKind } from "@/core/knowledge/fabric";
export const runtime = "nodejs"; export const dynamic = "force-dynamic";

/** GET ?q=&workspace=&k=&asOf=&mode=&entity=&tag= → hybrid hits (or recent facts + status when no q). POST {text, workspace?, tags?, source?, ref?, confidence?, validFrom?, validTo?, supersedes?, edges?}. */
export async function GET(req: Request) {
  ...
  let body: unknown;
  try {
    if (q) body = { hits: await queryFacts(uid, q, { workspace: ws, k: Number(u.searchParams.get("k") ?? 8), asOf: u.searchParams.get("asOf") ? Number(u.searchParams.get("asOf")) : undefined, mode: (u.searchParams.get("mode") as "hybrid" | undefined) ?? undefined, entity: u.searchParams.get("entity") ?? undefined, tags: u.searchParams.getAll("tag") }) };
