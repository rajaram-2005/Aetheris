    ...
    <div className="login-wrap">
      <div className="login-card">
        ...
        <div className="mode-toggle" style={{ marginBottom: 12, justifyContent: "center" }}>
          <button className={tab === "email" ? "active" : ""} onClick={() => { setTab("email"); setStage("enter"); setErr(null); }}>✉️ {t("login.email")}</button>
          <button className={tab === "phone" ? "active" : ""} onClick={() => { setTab("phone"); setStage("enter"); setErr(null); }}>📱 {t("login.phone")}</button>
        </div>
        ...
        {stage === "enter" ? (
          <form className="login-form" onSubmit={start}>
            ...
            <button className="send" disabled={busy || !target.trim()}>{busy ? t("login.sending") : tab === "email" ? t("login.sendEmail") : t("login.sendSms")}</button>
          </form>
        ) : (
          <form className="login-form" onSubmit={verify}>
            <div className="hint" style={{ margin: 0 }}>Code sent to <b>{sentTo}</b>. <button type="button" className="link" onClick={() => { setStage("enter"); setCode(""); }}>{t("login.change")}</button></div>
            {devCode && <div className="upsell">Dev mode (no {tab === "email" ? "email" : "SMS"} provider configured) — your code is <code>{devCode}</code></div>}
            <input autoFocus inputMode="numeric" pattern="[0-9 ]*" maxLength={7} placeholder={t("login.code")} value={code} onChange={(e) => setCode(e.target.value)} required className="login-code" />
            <input placeholder={t("login.name")} value={name} onChange={(e) => setName(e.target.value)} />
            <button className="send" disabled={busy || code.replace(/\D/g, "").length !== 6}>{busy ? t("login.verifying") : t("login.signIn")}</button>
          </form>
        )}

        {err && <div className="err-text" style={{ marginTop: 10 }}>{err}</div>}
        <p className="hint" style={{ marginTop: 18 }}>By continuing you agree to fair use of the free tiers. <a href="/">{t("login.guest")}</a></p>
      </div>
    </div>
  );
}
